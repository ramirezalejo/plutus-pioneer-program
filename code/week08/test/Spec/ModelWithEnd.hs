...
import           Week08.TokenSaleWithEnd                   (TokenSale (..), TSStartSchema', TSUseSchema, TSCloseSchema, startEndpoint', useEndpoints, closeEndpoint, nftName)
...
instance ContractModel TSModel where
    ...
    data ContractInstanceKey TSModel w s e where
        ...
        CloseKey :: Wallet -> Wallet -> ContractInstanceKey TSModel ()               TSCloseSchema  Text
    ...
    perform h _ cmd = case cmd of
        ...
        (Close    v w)     -> callEndpoint @"close"      (h $ CloseKey v w) ()                                                                    >> delay 1
...
instanceSpec =
    ...
    [ContractInstanceSpec (CloseKey v w) w $ closeEndpoint $ tss Map.! v | v <- wallets, w <- wallets]
