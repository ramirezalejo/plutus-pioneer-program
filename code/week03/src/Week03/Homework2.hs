...
mkValidator beneficiary deadline _ ctx = checkSig && checkDeadline
  where
    info :: TxInfo
    ...
    checkDeadline :: Bool
    checkDeadline = traceIfFalse "beneficiary's deadline not reached" (before deadline (txInfoValidRange info)) 
